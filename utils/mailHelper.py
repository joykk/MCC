...
class mailHelper(object):
    ...
    def analysisMail(self, mailBody):
        ...
        try:
            tempstr=str(mailBody[1]).decode("gb18030")
            #print chardet.detect(tempstr)
            subject = re.search("'Subject: (.*?)'",tempstr,re.S).group(1)
            print "command is", subject
            sender = re.search("'X-Sender: (.*?)'",tempstr,re.S).group(1)
